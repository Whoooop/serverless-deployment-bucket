...
class DeploymentBucketPlugin {
  ...
  async hasBucketEncryption(name, sseAlgorithm, kmsMasterKeyId) {
    const params = {
      Bucket: name
    };

    try {
      const data = await this.provider.request('S3', 'getBucketEncryption', params);
      if(data?.ServerSideEncryptionConfiguration?.Rules && data?.ServerSideEncryptionConfiguration?.Rules.length>0) {
        const rule = data?.ServerSideEncryptionConfiguration?.Rules[0].ApplyServerSideEncryptionByDefault;

        if(rule.SSEAlgorithm !== sseAlgorithm) {
            this.serverless.cli.log(`sseAlgorithm mismatch wanted: ${sseAlgorithm} current: ${rule.SSEAlgorithm}`);
            return false
        }
        if(rule.SSEAlgorithm === 'aws:kms' && rule.KMSMasterKeyID !== kmsMasterKeyId) {
            this.serverless.cli.log(`kmsMasterKeyId mismatch wanted: ${kmsMasterKeyId} current: ${rule.KMSMasterKeyID}`);
            return false
        }
      }
  ...
  async applyDeploymentBucket() {
    try {
      ...
      if (this.deploymentBucket.serverSideEncryption) {
        if (!(await this.hasBucketEncryption(this.deploymentBucket.name, this.deploymentBucket.serverSideEncryption, this.deploymentBucket.sseKMSKeyId))) {
          if (this.deploymentBucket.serverSideEncryption === "aws:kms") {
            await this.putBucketEncryption(this.deploymentBucket.name, this.deploymentBucket.serverSideEncryption, this.deploymentBucket.sseKMSKeyId)
          }
